...
public class SchoolServiceImpl extends AuthenticationFacade implements SchoolService {
    ...
    public School selectRandomSchool() {
        ...
        School school = schoolRepository.selectRandomSchool(user.getSchool().getId().isEmpty() ? "" : user.getSchool().getId()).orElseThrow(() -> new NotFoundException("Random not running"));
